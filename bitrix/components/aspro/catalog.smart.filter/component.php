...
if($this->StartResultCache(false, 'v2'.($arParams["CACHE_GROUPS"]? $USER->GetGroups(): false)))
{
	...
	{
		if ($this->facetAspro->isValid())
		{
			$this->facetAspro->setPrices($arResult["PRICES"]);
			...
			$res = $this->facetAspro->query($arResult["FACET_FILTER"]);
			...
			while ($row = $res->fetch())
			{
				...
				if (\Bitrix\Iblock\PropertyIndex\Storage::isPropertyId($facetId))
				{
					...
					{
						$addedKey = $this->fillItemValues($arResult["ITEMS"][$PID], $this->facetAspro->lookupDictionaryValue($row["VALUE"]), true);
						if ($addedKey)
						{
...
{
	$this->facetAspro->setPrices($arResult["PRICES"]);
	$this->facetAspro->setSectionId($this->SECTION_ID);
}
	...
	foreach($arItem["VALUES"] as $key => $ar)
	{
		...
		)
		{
			if($arItem["PROPERTY_TYPE"] == "N")
			{
				...
				{
					if ($key == "MIN")
						$this->facetAspro->addNumericPropertyFilter($PID, ">=", $_CHECK[$ar["CONTROL_NAME"]]);
					elseif ($key == "MAX")
						$this->facetAspro->addNumericPropertyFilter($PID, "<=", $_CHECK[$ar["CONTROL_NAME"]]);
				}
			}
			elseif(isset($arItem["PRICE"]))
			{
				...
				{
					if ($key == "MIN")
						$this->facetAspro->addPriceFilter($arResult["PRICES"][$PID]["ID"], ">=", $_CHECK[$ar["CONTROL_NAME"]]);
					elseif ($key == "MAX")
						$this->facetAspro->addPriceFilter($arResult["PRICES"][$PID]["ID"], "<=", $_CHECK[$ar["CONTROL_NAME"]]);
				}
			}
			elseif($arItem["DISPLAY_TYPE"] == "U")
			{
				...
				{
					if ($key == "MIN")
						$this->facetAspro->addDatetimePropertyFilter($PID, ">=", MakeTimeStamp($_CHECK[$ar["CONTROL_NAME"]], FORMAT_DATE));
					elseif ($key == "MAX")
						$this->facetAspro->addDatetimePropertyFilter($PID, "<=", MakeTimeStamp($_CHECK[$ar["CONTROL_NAME"]], FORMAT_DATE) + 23*3600+59*60+59);
				}
			}
			elseif($_CHECK[$ar["CONTROL_NAME"]] == $ar["HTML_VALUE"])
			{
				...
				if ($arResult["FACET_FILTER"])
				{
					if ($arItem["USER_TYPE"] === "DateTime")
						$this->facetAspro->addDatetimePropertyFilter($PID, "=", MakeTimeStamp($ar["VALUE"], FORMAT_DATE));
					else
						$this->facetAspro->addDictionaryPropertyFilter($PID, "=", $ar["FACET_VALUE"]);
				}
			}
...
if ($_CHECK)
{
	...
	if ($arResult["FACET_FILTER"])
	{
		if (!$this->facetAspro->isEmptyWhere())
		{
			foreach ($arResult["ITEMS"] as $PID => &$arItem)
			{
			...
			if ($arResult["CURRENCIES"])
				$this->facetAspro->enableCurrencyConversion($this->convertCurrencyId, array_keys($arResult["CURRENCIES"]));
				...
				else
				{
					...
					foreach($arResult["PRICES"] as $NAME => $arPrice)
					{
						...
						)
						{
							...
							if ($existCurrency)
								$currency = $this->facetAspro->lookupDictionaryValue($currency);
...
foreach($arResult["ITEMS"] as $PID => $arItem)
{
	if (!isset($arItem["NAME"])) unset($arResult["ITEMS"][$PID]);
