...
\Bitrix\Main\Loader::includeModule('aspro.popup');
...
$arParams["IS_AJAX"] = (Aspro\Popup\General::checkAjaxRequest() && $arParams['SHOW_FORM'] == 'Y');
...
if(!$bError && \Bitrix\Main\Loader::includeModule('aspro.popup'))
{
	if($arParams["IS_AJAX"] != 'Y' && !$isWebform)
	{
		$ob = new Aspro\Popup\MarketingPopup();
		$rules = $ob->GetRules();
