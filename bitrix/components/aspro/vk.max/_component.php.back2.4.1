...
if(\Bitrix\Main\Loader::includeModule('aspro.max')){
	...
	$arResult['TOKEN'] = ( strlen($arParams["API_TOKEN_VK"]) > 0 ? $arParams["API_TOKEN_VK"] : CMax::GetFrontParametrValue('API_TOKEN_VK') );
	...
	if(
		...
		$obVK = new CVKMax($arResult['TOKEN'], $arParams['PAGE_ELEMENT_COUNT']);
