...
class SmartseoTagsComponent extends \CBitrixComponent
{
    ...
    {
        if (!class_exists('\Aspro\Smartseo\General\Smartseo') || !Smartseo\General\Smartseo::validateModules()) {
    ...
    protected function getTagTableRows($filter = [])
    {
        ...
        foreach ($rows as $row) {
            $result[] = [
                ...
                'RELATED_PROPERTY' => $row['RELATED_PROPERTY'] ? Smartseo\General\Smartseo::unserialize($row['RELATED_PROPERTY']) : [],
    ...
    protected function getPreparePropertyValues($propertySerialized = '')
    {
        ...
        $urlProperties = Smartseo\General\Smartseo::unserialize($propertySerialized);
