if(typeof window.JCBasketShareDetail === 'undefined'){
	...
	window.JCBasketShareDetail.prototype = {
		...
		updateServicesButtons: function(itemInput){
			//var _this = $(this);
			var parentItem = itemInput.closest('.basket-share-detail__item');
			var itemOn = itemInput.prop('checked');
			var services = parentItem.find('.basket-share-service__item__check input[type=checkbox]');
			if(services.length){
				if(!itemOn){
		...
		updateAllServicesButtons: function(){
			var itemInputs = this.$form.find('.basket-share-detail__item__check input[type=checkbox]');
			if(itemInputs.length){
				for(var i = 0 ; i<itemInputs.length; i++){
					this.updateServicesButtons($(itemInputs[i]));
				}
			}
		},
			...
			if(this.$block.length){
				if(this.$form.length){
					...
					this.$form.find('.basket-share-detail__item__check input[type=checkbox]').change(function(){
						...
						that.updateServicesButtons( $(this) );						
					});
