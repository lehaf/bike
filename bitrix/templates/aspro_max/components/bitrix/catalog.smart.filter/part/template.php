...
<form action="#" id="filter" class="selection-block" method="get" data-sect="<?= $arResult['SECTION']['ID'] ?>"
    ...
    <div class="form-header">
        ...
        <!--        состояние-->
        <?php if (!empty($arResult['ITEMS']['category_part']['VALUES'])): ?>
            <div class="form-row form-row-radio-block">
                <?php foreach ($arResult['ITEMS']['category_part']['VALUES'] as $key => $ar): ?>
                    <div class="form-col">
                        <input type="radio"
                               class="radio-block"
                            ...
                            <?= ($ar["CHECKED"] || $key === array_key_first($arResult['ITEMS']['category_part']['VALUES'])) ? 'checked' : '' ?>
                        >
    ...
    <div class="form-row flex-row">
        ...
        <?php endif; ?>
        <?php if (!empty($arResult['ITEMS']['type_zapchasti_search']['VALUES'])): ?>
            <div class="form-row__col">
                <div class="custom-select-inner form-group-custom-select">
                    <div class="custom-select--multiple">
                        <select name="<?= current($arResult['ITEMS']['type_zapchasti_search']['VALUES'])['CONTROL_NAME_ALT'] ?>"
                                class="select-type custom-select">
                            <option value="" selected>
                                Выбрать запчасть
                            </option>
                            ...
                            <?php foreach ($arResult['ITEMS']['type_zapchasti_search']['VALUES'] as $ar): ?>
                                <option
                                        id="<?= $ar['CONTROL_ID'] ?>"
                                        value="<?= $ar["HTML_VALUE_ALT"] ?>"
                                    <?= ($ar["CHECKED"]) ? 'selected' : '' ?>
...
<path d="M8.98109 1.35706L1.69526 8.64289M9.03313 8.69493L1.64322 1.30502" stroke="#ED1C24" stroke-width="1.4"
    ...
    <div class="form-row form-row--btn form-row--end">
