			...
			<?foreach($arResult['ITEMS'] as $arItem):?>
				<div class="item" style="font-size:0px;color: #000;box-sizing: border-box;text-align:left;padding:0px 0px 25px;margin-bottom:25px;<?=($i == $count ? 'margin-bottom:0px;' : 'border-bottom:1px solid #dedede;')?><?=($i == 1 ? 'margin-top:0px;' : 'margin-top:25px;')?>">
					<?if($arItem["PREVIEW_PICTURE"]["SRC"]):
						...
						<div class="img" style="max-width: 130px;margin:0px;font-size: 12px;width: 100%;display: inline-block;vertical-align: top;padding: 0px 20px 20px 0px"><img src="<?=$img["src"]?>" alt="<?=$arItem["NAME"];?>" title="<?=$arItem["NAME"];?>" style="max-width: 100%;"/></div>
						<div class="right-data" style="padding: 0 0px 2px;margin-left: 0;display: inline-block;vertical-align: top;width: 100%;max-width: 389px;box-sizing: border-box;">
					<?endif;?>
